<!--
  horario_cuadrantes.php — Vista del cuadrante de horarios (admin)
  Incluida por panel.php (seccion=horario&vista=cuadrantes). Solo admin.
  Muestra una tabla semanal/mensual con los horarios de todos los empleados.
  Incluye horario_cuadrantes_scripts.php que gestiona la interactividad AJAX.
-->
        ...
        <thead>
            <tr>
                ...
                foreach ($fechas as $fecha):
                    $ts    = strtotime($fecha);
                    $dia   = $diasES[date('N', $ts)];
                    $num   = date('d', $ts);
                    $mes   = $mesesES[date('n', $ts)];
                    $esFin = date('N', $ts) >= 6;
                ?>
...
<?php include __DIR__ . '/horario_cuadrantes_scripts.php'; ?>