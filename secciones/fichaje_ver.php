...
// 2. CAPTURA DE FILTROS (Mantenemos coherencia con tu panel.php)
$modo = $_GET['modo'] ?? 'dia';
...
// 3. DETERMINAR RANGO REAL DE BÚSQUEDA
...
// Usamos LEFT JOIN para ver quién ha fichado y quién no en el rango
$sql = "SELECT 
            U.id_usuario, U.nombre, U.apellidos,
            F.fecha, F.id_fichaje, F.hora_entrada, F.hora_pausa, 
        ...
        LEFT JOIN FICHAJE F ON U.id_usuario = F.id_usuario AND F.fecha BETWEEN :inicio AND :fin
        LEFT JOIN HORARIOS H ON U.id_usuario = H.id_usuario 
            AND H.id_empresa = EU.id_empresa 
            AND H.fecha = F.fecha
        WHERE EU.id_empresa = :empresa AND EU.activo = 1";

if ($empleado_filtro) {
    $sql .= " AND U.id_usuario = :id_empleado";
}

$sql .= " ORDER BY F.fecha $orden, F.hora_entrada $orden, U.nombre ASC";
...
// Agrupar por fecha para la interfaz
$registrosPorFecha = [];
...
$stmtEmp = $pdo->prepare("SELECT U.id_usuario, U.nombre, U.apellidos FROM USUARIO U 
                         JOIN EMPRESA_USUARIO EU ON U.id_usuario = EU.id_usuario 
                         WHERE EU.id_empresa = ? ORDER BY U.nombre ASC");
...
<div class="fichaje-wrapper">
    <div class="section-header">
        <h2><i class="fas fa-list-alt"></i> Registros de Fichaje</h2>
    ...
    <?php else: ?>
        <?php foreach ($registrosPorFecha as $fecha => $regs): ?>
                    ...
                    <table class="f-table">
                        ...
                        <tbody>
                            <?php foreach ($regs as $r): ?>
                                <tr>
                                    <td class="name-td"><strong><?= htmlspecialchars($r['nombre'] . ' ' . $r['apellidos']) ?></strong></td>
                                    <td><?= $r['hora_entrada'] ? substr($r['hora_entrada'], 0, 5) : '--:--' ?></td>
                                    <td><?= $r['hora_salida'] ? substr($r['hora_salida'], 0, 5) : '--:--' ?></td>
                                    <td>
                                        <?php if(!$r['hora_entrada']): ?>
                                            <span class="st-badge ausente">Sin fichar</span>
                                        <?php elseif($r['hora_entrada'] && !$r['hora_salida']): ?>
                                            <span class="st-badge proceso">En curso</span>
                                        <?php else: ?>
                                            <span class="st-badge ok">Finalizado</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="obs-td"><?= htmlspecialchars($r['observaciones'] ?? '') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
