<?php
/*
 * empleados_lista.php — Vista del listado de empleados (admin)
 * Incluida por panel.php (seccion=empleados&vista=lista). Solo admin.
 * Muestra foto, nombre, email y estado de cada empleado de la empresa activa.
 * Las acciones de editar, activar/desactivar y eliminar se ejecutan via fetch AJAX
 * a empleado_obtener.php, empleado_actualizar.php, empleado_cambiar_estado.php y empleado_eliminar.php.
 */
                    ...
                    <tr class="<?= $emp['empresa_activo'] == 0 ? 'empleado-inactivo' : '' ?>">
                        ...
                        <td>
                            <?php if ($emp['empresa_activo'] == 1): ?>
                                <span class="badge badge-activo"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="1em" height="1em" style="vertical-align:-0.125em"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg> Activo</span>
                            <?php else: ?>
                                <span class="badge badge-inactivo"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="1em" height="1em" style="vertical-align:-0.125em"><path stroke-linecap="round" stroke-linejoin="round" d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg> Inactivo</span>
                            <?php endif; ?>
                        </td>
                        <td class="acciones-col">
                            <button class="btn-action btn-edit" onclick="editarEmpleado(<?= $emp['id_usuario'] ?>)" title="Editar"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="1em" height="1em"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125"/></svg></button>
                            <?php if ($emp['empresa_activo'] == 1): ?>
                                <button class="btn-action btn-deactivate" onclick="cambiarEstadoEmpleado(<?= $emp['id_usuario'] ?>, 0)" title="Desactivar"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="1em" height="1em"><path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 0 0 5.636 5.636m12.728 12.728A9 9 0 0 1 5.636 5.636m12.728 12.728L5.636 5.636"/></svg></button>
                            <?php else: ?>
                                <button class="btn-action btn-activate" onclick="cambiarEstadoEmpleado(<?= $emp['id_usuario'] ?>, 1)" title="Activar"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="1em" height="1em"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg></button>
                            <?php endif; ?>
                            <button class="btn-action btn-delete" onclick="eliminarEmpleado(<?= $emp['id_usuario'] ?>)" title="Eliminar"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="1em" height="1em"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/></svg></button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
...
function editarEmpleado(idUsuario) {
    fetch(`secciones/empleados/empleado_obtener.php?id=${idUsuario}`)
        .then(res => res.json())
        .then(data => {
...
function guardarCambios(event) {
    ...
    fetch('secciones/empleados/empleado_actualizar.php', {
        method: 'POST',
...
function cambiarEstadoEmpleado(idUsuario, nuevoEstado) {
    ...
    fetch('secciones/empleados/empleado_cambiar_estado.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
function eliminarEmpleado(idUsuario) {
    ...
    fetch('secciones/empleados/empleado_eliminar.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
