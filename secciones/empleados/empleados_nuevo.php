<?php
/*
 * empleados_nuevo.php — Vista del formulario de alta de empleado (admin)
 * Incluida por panel.php (seccion=empleados&vista=nuevo). Solo admin.
 * Recoge nombre, apellidos, email, contraseña y NIF del nuevo empleado.
 * Verifica email duplicado en tiempo real (fetch a empleado_verificar_email.php).
 * Envía el alta via fetch POST a empleado_crear.php.
 */
...
function guardarNuevoEmpleado(event) {
    ...
    fetch('secciones/empleados/empleado_crear.php', {
        method: 'POST',
...
document.getElementById('nuevo_email').addEventListener('blur', function() {
    if (this.value) {
        fetch(`secciones/empleados/empleado_verificar_email.php?email=${encodeURIComponent(this.value)}`)
            .then(r => r.json())
