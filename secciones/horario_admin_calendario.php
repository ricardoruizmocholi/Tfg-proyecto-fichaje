<?php
// secciones/horario_admin_calendario.php
// Calendario mensual del admin — guarda DIRECTAMENTE en HORARIOS (sin solicitud)
// El admin puede ver y editar el horario de cualquier empleado desde esta vista.
...
async function guardarDiaAdmin() {
    ...
    mostrarLoadingAdmin(true);
    try {
        const r = await fetch('secciones/horario_guardar.php', {
            method: 'POST',
...
async function eliminarHorarioAdmin(idHorario) {
    mostrarLoadingAdmin(true);
    try {
        const r = await fetch('secciones/horario_eliminar.php', {
...
async function guardarMasivoAdmin() {
    ...
    mostrarLoadingAdmin(true);
    try {
        const r = await fetch('secciones/horario_guardar_masivo_admin.php', {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({
